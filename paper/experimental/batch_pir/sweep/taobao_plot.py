...
def plot_computation_vs_accuracy(data):
    ...
    plt.tight_layout()
    plt.savefig(f"taobao_computation_vs_auc.pdf", tight_layout=True)        

def plot_communication_vs_accuracy(data):
    accuracy_baseline = max([x["accuracy_stats"]["auc"] for x in data])
    
    data = [x for x in data if x["cost"]["computation"] <= 100000000000]
    #data = [x for x in data if (x["cost"]["upload_communication"]+x["cost"]["download_communication"])/1000 < 150]
    data = [x for x in data if x["accuracy_stats"]["auc"] >= .58]
    ...
    plt.legend(loc="best", fontsize=14)
    plt.tight_layout()
    plt.savefig(f"taobao_communication_vs_auc.pdf", tight_layout=True)            
...
def plot_communication_vs_computation_cost(data):
    ...
    plt.legend(loc="best", fontsize=14)
    plt.tight_layout()
    plt.savefig(f"taobao_communication_vs_computation.pdf", tight_layout=True)        
