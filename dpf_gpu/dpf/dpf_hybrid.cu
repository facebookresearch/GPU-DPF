...
__global__ void dpf_hybrid_kernel(SeedsCodewordsFlatGPU *cw, uint128_t_gpu *out,
  ...
  while (stack_indx >= 0) {
	...
	if (expansion_idx < Z) {
	  ...
	  for (int z = 0; z < MM; z++) {
	    per_thread_accumulate[z] = add_uint128(mul_uint128(dpf_output, TABLE[write_out + expansion_idx + z*num_entries]), per_thread_accumulate[z]);
	  }
#endif
	}
      }
	...
	if (expansion_idx < end) {
	  ...
	  uint128_t_gpu key = key_read[cw_read_write_indx_strt + (expansion_idx/2) +
				       (block_idx+batch_idx)*max_stack_size_per_dpf];
	  uint128_t_gpu value = PRF<prf_method>(key, idx_into_codewords);
	  uint128_t_gpu *cw = (key.x & 1) == 0 ? cw_shared[batch_idx].cw_1 : cw_shared[batch_idx].cw_2;
...
void dpf_hybrid(SeedsCodewordsFlatGPU * cw,
  ...
  dpf_hybrid_kernel<prf_method><<<n_blocks, n_threads, 0, s>>>(cw, out,
							       DPF_HYBRID_STACK_1,
							       DPF_HYBRID_STACK_2,
							       TABLE,
							       batch_size,
